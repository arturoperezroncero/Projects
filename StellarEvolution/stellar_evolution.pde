...
void draw(){
  ...
  if(mode==0){
    ...
    fill(255);
    textAlign(CENTER);
    text("MEDIUM MASS",width/10*3+50,(height/5*3)-100);
    ...
    fill(255);
    textAlign(CENTER);
    text("HIGH MASS",width/10*5,(height/5*3)-150);
    ...
    fill(255);
    textAlign(CENTER);
    text("STELLAR EVOLUTION",(width/2)-50+mouseX/10,100+mouseY/10);
  ...
  if(mode>0){
    ...
    imageMode(CENTER);
    exit.resize(40,40);
    image(exit,width/20*19,height-60);
  }
  if(mouseX>(width/20*19)-40 && mouseX<(width/20*19)+40 && mouseY>height-80 && mouseY<height-20 && mousePressed){
    mode=0;
  }
...
void mousePressed(){
  if(mouseX>75 && mouseX<200 && mouseY>620 && mouseY<750){
   mode=5;
 }
}